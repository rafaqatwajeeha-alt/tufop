  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" className="bg-white text-[#6DC7B8] font-bold h-10 px-6 rounded-xl shadow-xl">
      ...
      <div className="flex gap-4">
        <div className="relative flex-1">
          ...
          <Input className="pl-10 bg-white/10 border-white/20 text-white placeholder:text-white/40 focus:border-white/40 focus:ring-white/10 rounded-xl" placeholder="Search programs..." />
        </div>
            ...
            {data?.programs.map((prog) => (
              <TableRow key={prog.id} className="border-white/5 hover:bg-white/5 transition-colors">
                ...
                <TableCell className="text-right">
                  <Button variant="ghost" size="icon" className="h-10 w-10 text-white/50 hover:text-white hover:bg-white/10 rounded-xl transition-all"><MoreVertical className="h-5 w-5" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
